...
from src.views_registry import VIEWS

def get_current_semester(notion: NotionClient) -> Optional[str]:
    ...
    semesters = notion.query_data_source(settings.notion_semesters_ds_id, _filter, _sort)
...
def reconcile_semester_status(notion: NotionClient, semester_page_id: str) -> None:
    """Demotes any Current semester (other than the target) to Past, then promotes the target to Current."""

    current_semesters = notion.query_data_source(
        settings.notion_semesters_ds_id,
        filter_dict={"property": "Status", "select": {"equals": "Current"}},
    )

    for semester in current_semesters:
        if semester["id"] != semester_page_id:
            notion.update_page(semester["id"], {"Status": notion_props.select("Past")})

    notion.update_page(semester_page_id, {"Status": notion_props.select("Current")})
    
    
def update_view_filters(notion: NotionClient, semester_page_id: str) -> None:
    ...
    for view in VIEWS:
        ...
        notion.update_view(view["view_id"], combined_filter)